...
def compact_rectification_for_ai(rectification):
    """Сжимает результат ректификации перед отправкой в AI, чтобы не перегружать API."""
    if not isinstance(rectification, dict):
        return {}

    top = []
    for item in (rectification.get("top_variants") or [])[:7]:
        summary = item.get("summary") or {}
        top.append({
            "time": item.get("time"),
            "total_score": item.get("total_score"),
            "lagna": summary.get("lagna"),
            "varga_lagnas": summary.get("varga_lagnas"),
            "karakas": summary.get("karakas"),
            "event_scores": [
                {
                    "event": es.get("event"),
                    "event_label": es.get("event_label"),
                    "score": es.get("score"),
                    "vimshottari_path": es.get("vimshottari_path"),
                    "details": es.get("details"),
                }
                for es in (item.get("event_scores") or [])
            ],
        })

    return {
        "date": rectification.get("date"),
        "city": rectification.get("city"),
        "step_minutes": rectification.get("step_minutes"),
        "variants_count": rectification.get("variants_count"),
        "best_time": rectification.get("best_time"),
        "best_score": rectification.get("best_score"),
        "confidence": rectification.get("confidence"),
        "top_variants": top,
        "corridors": (rectification.get("corridors") or [])[:12],
    }
...
@app.route("/api/ai_rectification", methods=["POST"])
def ai_rectification():
    print("POST /api/ai_rectification started", flush=True)

    data = request.get_json(force=True) or {}
    rectification = data.get("rectification") or {}
    question = data.get("question", "").strip() or "Проанализируй результат ректификации и объясни лучший коридор времени."

    if not OPENROUTER_API_KEY:
        return jsonify({
            "success": False,
            "error": "OPENROUTER_API_KEY не задан в переменных Amvera. Без него AI-анализ ректификации недоступен."
        })

    compact = compact_rectification_for_ai(rectification)

    if not compact.get("top_variants"):
        return jsonify({
            "success": False,
            "error": "Нет данных ректификации для AI-анализа. Сначала запустите ректификацию."
        })

    prompt = f"""
Ты Аная — профессиональный AI-астролог AstroEngine в системе ведической астрологии Джйотиш.

Задача: проанализировать результаты ректификации времени рождения.

Жёсткие правила:
1. Не пересчитывай карту самостоятельно.
2. Не выдумывай данные, которых нет в переданном результате.
3. Используй только рассчитанные варианты времени, лагны D1/D7/D9/D10/D24, Вимшоттари и события.
4. Если уверенность низкая, прямо скажи об этом.
5. Раздели ответ на блоки: лучший вариант, коридоры дробных карт, подтверждения событиями, слабые места, что уточнить дополнительно.
6. Пиши по-русски, профессионально, но понятно.
...
"""
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"OpenRouter rectification error: {result}", flush=True)
        ...
        if not answer:
            return jsonify({"success": False, "error": "OpenRouter не вернул текст ответа."})

        print("POST /api/ai_rectification success", flush=True)
        return jsonify({"success": True, "answer": answer})

    except Exception as e:
        print(f"AI rectification error: {e}", flush=True)
...
def save_chart_route():
    ...
    # Сохраняем карту по введённым данным, без привязки к Telegram ID.
    # В PostgreSQL поле telegram_user_id имеет тип BIGINT, поэтому пустую строку туда писать нельзя.
...
def saved_charts_route():
    ...
    # Показываем все сохранённые карты, без фильтра по Telegram ID.
    try:
        charts = get_saved_charts(None)
...
def open_chart_route(chart_id):
    ...
    try:
        ...
        chart["telegram_user_id"] = saved.get("telegram_user_id")
        chart["active_vimshottari"] = extract_current_dasha_text(chart.get("dashas", {}))
        chart["active_chara_dasha"] = extract_current_dasha_text(chart.get("chara_dasha", {}))
