...
def calculate_mandi_gulika(local_dt, lat, lon, timezone_name):
    ...
    jyotish_weekday = py_to_jyotish_weekday[weekday]
    ...
    if sunrise_today <= jd_birth < sunset_today:
        ...
        gulika_jd = sunrise_today + part * segment_index
        mandi_jd = gulika_jd
    else:
        ...
        gulika_jd = night_start + part * segment_index
        mandi_jd = gulika_jd

    gulika_lon = calc_lagna_longitude(gulika_jd, lat, lon)
    mandi_lon = calc_lagna_longitude(mandi_jd, lat, lon)
...
def chara_sign_direction(sign_index):
...
def chara_sign_sequence(start_sign_index):
    direction = chara_sign_direction(start_sign_index)

    if direction == "forward":
        return [(start_sign_index + i) % 12 for i in range(12)]

    return [(start_sign_index - i) % 12 for i in range(12)]


def chara_distance_rao(from_sign_index, to_sign_index):
    direction = chara_sign_direction(from_sign_index)

    if direction == "forward":
        distance = ((to_sign_index - from_sign_index) % 12) + 1
    else:
        distance = ((from_sign_index - to_sign_index) % 12) + 1
...
def chara_period_years_rao(sign_index, planets):
    ...
    return chara_distance_rao(sign_index, lord_sign_index)


def make_chara_node(sign_index, start_dt, end_dt, level, path):
    sign_name = SIGNS[sign_index]
...
def build_chara_children_rao(parent_start, parent_end, parent_sign_index, level, path, max_depth):
    total_days = (parent_end - parent_start).total_seconds() / 86400
    part_days = total_days / 12
    children = []
    current_start = parent_start
    sequence = chara_sign_sequence(parent_sign_index)

    for index, sign_index in enumerate(sequence):
        current_end = add_days(current_start, part_days)
...
def find_current_chara_path(nodes):
    for node in nodes:
        if node.get("is_current"):
            return node["path"]

        child_path = find_current_chara_path(node.get("children", []))
...
def calculate_chara_dasha_rao(local_dt, lagna_sign_index, planets):
    periods = []
    current_start = local_dt.replace(tzinfo=None)
    sequence = chara_sign_sequence(lagna_sign_index)

    for sign_index in sequence:
        years = chara_period_years_rao(sign_index, planets)
    ...
    return {
        ...
        "current_path": find_current_chara_path(periods),
        "tree": periods,
        "mahadashas": periods
    }
...
def calculate_simple_chara_dasha(local_dt, lagna_sign_index):
    signs_order = list(range(lagna_sign_index, 12)) + list(range(0, lagna_sign_index))
    periods = []
    current_start = local_dt.replace(tzinfo=None)

    for sign_index in signs_order * 10:
        years = 9
        current_end = current_start + timedelta(days=years * 365.2425)
        periods.append({
            "sign": SIGNS[sign_index],
            "sign_number": SIGN_NUMBERS[SIGNS[sign_index]],
            "start": format_date(current_start),
            "end": format_date(current_end),
            "is_current": period_is_current(current_start, current_end)
        })
        current_start = current_end
