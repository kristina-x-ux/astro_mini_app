...
def build_chara_children_rao(parent_start, parent_end, parent_sign_index, level, path, max_depth):
    ...
    for index, sign_index in enumerate(chara_sequence_from(parent_sign_index)):
...
def calculate_chara_dasha_rao(local_dt, lagna_sign_index, planets, cycles=1):
    ...
    for _cycle in range(cycles):
        for sign_index in chara_sequence_from(lagna_sign_index):
    ...
    if periods:
        last_end = datetime.strptime(periods[-1]["end"], "%d.%m.%Y")
