...
def summarize_rectification_chart(chart):
    ...
    return {
        ...
        "varga_lagnas": {
            ...
            if name in ["D1", "D7", "D9", "D10", "D24", "D30"]
        },
        "karakas": {
...
def detect_rectification_corridors(variants):
    ...
    for item in variants:
        ...
        key = tuple((name, lagnas.get(name, {}).get("lagna_sign")) for name in ["D1", "D9", "D7", "D10", "D24"])
        ...
        if key != last_key:
            ...
            current = {
                ...
                "varga_lagnas": {name: lagnas.get(name, {}).get("lagna_sign") for name in ["D1", "D9", "D7", "D10", "D24"]},
                "times": [item.get("time")],
            }
            last_key = key
        else:
...
def calculate_rectification_range(date_str, start_time, end_time, city, life_events=None, step_minutes=15, lat=None, lon=None, display_name=None):
    ...
    Это базовый технический слой. Его можно безопасно отдавать AI для экспертной интерпретации.
    """
    ...
    for time_str in times:
        ...
        event_scores = [_score_event_against_chart(chart, event) for event in life_events]
    ...
    corridors = detect_rectification_corridors(sorted(variants, key=lambda x: x.get("time", "")))

    best = variants[0] if variants else None
    second_score = variants[1].get("total_score", 0) if len(variants) > 1 else 0
    confidence = 0
    if best and best.get("total_score", 0) > 0:
        gap = best.get("total_score", 0) - second_score
        confidence = 80 if gap >= 30 else 65 if gap >= 15 else 50 if gap >= 5 else 40
